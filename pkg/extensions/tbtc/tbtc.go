...
type tbtc struct {
	hostChain              chain.Handle
	tbtcApplication        chain.TBTCHandle
	keepManager            chain.BondedECDSAKeepManager
	blockCounter           corechain.BlockCounter
...
func newTBTC(
	...
	return &tbtc{
		hostChain:              hostChain,
		tbtcApplication:        tbtcApplication,
...
func (t *tbtc) monitorRetrievePubKey(
	...
	monitoringStartFn := func(
		...
		return t.tbtcApplication.OnDepositCreated(handler)
	}
	...
	monitoringStopFn := func(
		...
		return t.tbtcApplication.OnDepositRegisteredPubkey(func(depositAddress string) {
	...
	actFn := func(depositAddress string) error {
		err := t.tbtcApplication.RetrieveSignerPubkey(depositAddress)
		if err != nil {
			return err
		}
...
func (t *tbtc) monitorProvideRedemptionSignature(
	...
	monitoringStartFn := func(
		...
		return t.tbtcApplication.OnDepositRedemptionRequested(handler)
	}
	...
	monitoringStopFn := func(
		...
		signatureSubscription := t.tbtcApplication.OnDepositGotRedemptionSignature(
		...
		redeemedSubscription := t.tbtcApplication.OnDepositRedeemed(
			func(depositAddress string) {
				if t.waitDepositStateChangeConfirmation(
					depositAddress,
					initialDepositState,
	...
	actFn := func(depositAddress string) error {
		keep, err := t.tbtcApplication.Keep(depositAddress)
		if err != nil {
			return err
		}

		redemptionRequestedEvents, err := t.tbtcApplication.PastDepositRedemptionRequestedEvents(
		...
		err = t.tbtcApplication.ProvideRedemptionSignature(
			depositAddress,
...
func (t *tbtc) monitorProvideRedemptionProof(
	...
	monitoringStartFn := func(
		...
		return t.tbtcApplication.OnDepositGotRedemptionSignature(handler)
	}
	...
	monitoringStopFn := func(
		...
		redemptionRequestedSubscription := t.tbtcApplication.OnDepositRedemptionRequested(
			func(depositAddress string) {
				if t.waitDepositStateChangeConfirmation(
					depositAddress,
					initialDepositState,
	...
	actFn := func(depositAddress string) error {
		...
		err = t.tbtcApplication.IncreaseRedemptionFee(
			depositAddress,
...
func (t *tbtc) shouldMonitorDeposit(
	...
	hasInitialState, err := utils.ConfirmWithTimeoutDefaultBackoff(
		...
		func(ctx context.Context) (bool, error) {
			currentState, err := t.tbtcApplication.CurrentState(depositAddress)
			if err != nil {
				return false, err
			}
