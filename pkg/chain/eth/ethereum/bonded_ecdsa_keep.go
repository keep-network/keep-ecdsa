...
func (ec *EthereumChain) watchETHDistributedToMembers(
	keepContract *abi.BondedECDSAKeep,
	success func(event *abi.BondedECDSAKeepETHDistributedToMembers),
	fail func(err error) error,
) (subscription.EventSubscription, error) {
	eventChan := make(chan *abi.BondedECDSAKeepETHDistributedToMembers)

	eventSubscription, err := keepContract.WatchETHDistributedToMembers(
		nil,
		eventChan,
	)
	if err != nil {
		close(eventChan)
		return nil, fmt.Errorf(
			"failed to create watch for BondedECDSAKeepETHDistributedToMembers event: [%v]",
			err,
		)
	}
