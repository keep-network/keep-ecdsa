...
func init() {
	AvailableCommands = append(AvailableCommands, cli.Command{
		...
		Subcommands: []cli.Command{{
			...
			Name:      "master-bonded-e-c-d-s-a-keep-address",
			Usage:     "Calls the constant method masterBondedECDSAKeepAddress on the BondedECDSAKeepFactory contract.",
			ArgsUsage: "",
			Action:    becdsakfMasterBondedECDSAKeepAddress,
			Before:    cmd.ArgCountChecker(0),
			Flags:     cmd.ConstFlags,
		}, {
...
func becdsakfMasterBondedECDSAKeepAddress(c *cli.Context) error {
	contract, err := initializeBondedECDSAKeepFactory(c)
	if err != nil {
		return err
	}

	result, err := contract.MasterBondedECDSAKeepAddressAtBlock(
