...
// ParseBondedECDSAKeepABI parses the ABI
func ParseBondedECDSAKeepABI() (*abi.ABI, error) {
	parsed, err := abi.JSON(strings.NewReader(BondedECDSAKeepABI))
	if err != nil {
		return nil, err
	}
...
func (_BondedECDSAKeep *BondedECDSAKeepFilterer) TryParseLog(log types.Log) (eventName string, event interface{}, ok bool, err error) {
	eventName, ok, err = _BondedECDSAKeep.contract.LogEventName(log)
	if err != nil || !ok {
		return "", nil, false, err
	}

	switch eventName {
	case "ConflictingPublicKeySubmitted":
		event, err = _BondedECDSAKeep.ParseConflictingPublicKeySubmitted(log)
	case "ERC20RewardDistributed":
		event, err = _BondedECDSAKeep.ParseERC20RewardDistributed(log)
	case "ETHRewardDistributed":
		event, err = _BondedECDSAKeep.ParseETHRewardDistributed(log)
	case "KeepClosed":
		event, err = _BondedECDSAKeep.ParseKeepClosed(log)
	case "KeepTerminated":
		event, err = _BondedECDSAKeep.ParseKeepTerminated(log)
	case "PublicKeyPublished":
		event, err = _BondedECDSAKeep.ParsePublicKeyPublished(log)
	case "SignatureRequested":
		event, err = _BondedECDSAKeep.ParseSignatureRequested(log)
	case "SignatureSubmitted":
		event, err = _BondedECDSAKeep.ParseSignatureSubmitted(log)
	case "SlashingFailed":
		event, err = _BondedECDSAKeep.ParseSlashingFailed(log)
	}
	if err != nil {
		return "", nil, false, err
	}
