...
////
Epoch length whatever

Request keeps

If keep available in pool,


==== Keep pool

Keeps that aren't currently assigned to customers
are kept in the keep pool.
When a customer requests a keep
we try to provide it from the pool,
but this can't be guaranteed.

If the number of keeps in the pool falls below _minimum pool size_
we trigger a pool refill by creating another batch of keeps.

===== Batch adjustment

The time of the last refill is recorded.
Time between last refill and the request which reduces the pool below minimum
is the _epoch length_.

If the time since the last refill is less than _threshold epoch length_
we add one to the _current batch size_
up to the _maximum batch size_.
If the time since the last refill is more than the threshold,
we subtract one from the _current batch size_
down to the _minimum batch size_.

==== Pool refill

When the keep pool is below _minimum pool size_
any operator can trigger a pool refill.

When a pool refill is triggered,
the factory requests an entry from the Random Beacon,
the _selection seed_.
The operator performing the trigger call is reimbursed
for the gas cost of the call.

When the beacon has generated the entry,
it performs the callback from the request
which begins the signer selection.

The _selection seed_ is salted with some identifier
(e.g. the address of the factory)
to make sure that we get different outputs
than a possible beacon group selection.
(We should probably salt the beacon too)address of the factory)
to make sure that we get different outputs
than a possible beacon group selection.
(We should probably salt the beacon too,
just for consistency.)
The _salted seed_ is used as an input for ticket value calculation,
alongside operator address and virtual staker index.

The factory emits an event specifying that it

When a ticket is submitted,
for 

Keeps are created in a batch
where members are all selected at once
and paid their initial reward when keep creation is finished.
The money for this comes from the _keep creation pool_.

When a customer application requests a keep
they pay the factory a _keep recruitment fee_.
The _keep recruitment fee_ is paid into the _keep creation pool_.

The keep creation pool needs to be kept funded
so that we can create batches.
