...
is expected to be approximately 2^13^ to 2^14^.
Again taking the optimal case gives a height of 13 for a binary tree.
Updating a single node in the tree,
along with its path from the root,
would require up to 14 write operations at a cost of 70,000 gas.
Accessing a node in the tree
would similarly require 14 read operations at 11,200 gas.
