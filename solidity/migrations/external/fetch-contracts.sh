...
CONTRACTS_NAMES=("SortitionPoolFactory.json")

DESTINATION_DIR=$(realpath $(dirname $0)/contracts)
...
function fetch_contracts() {
  ...
  do
    gsutil -q cp gs://${CONTRACT_DATA_BUCKET}/sortition-pools/${CONTRACT_NAME} $DESTINATION_DIR
  done
}
